(() => {
  ...
  const colors = ['#ff4b94', '#48c7ff', '#ffd166', '#7ef067', '#af79ff', '#ff9b4a'];
  ...
  function showQr() {
    ...
    el.qrImage.src = `https://api.qrserver.com/v1/create-qr-code/?size=520x520&margin=10&data=${data}`;
    el.qrImage.onerror = () => toast('Не удалось загрузить QR-код');
  }
      ...
      ? list.map((player) => `
          <div class="player-list-item">
            ...
            <div class="player-list-main">
              ...
              <small>${escapeHtml(player.carName)}</small>
            </div>
            <div class="player-list-percent" style="color:${player.color}">${Math.round(player.progress)}%</div>
          </div>`).join('')
      : '<div class="no-players">Пока никого нет</div>';

    el.raceTracks.innerHTML = list.map((player, index) => {
      const progress = Math.max(0, Math.min(100, player.progress));
      const left = Math.min(78, progress * 0.78);
      return `
        <article class="track-row" style="--accent:${player.color}">
          <div class="track-left">
            <div class="track-rank">${index + 1}</div>
            <div class="track-name">${escapeHtml(player.name)}</div>
            <div class="track-brand">${escapeHtml(player.carName)}</div>
            <div class="track-progress-line"><div style="width:${Math.round(progress)}%"></div></div>
            <div class="track-percentage">${Math.round(progress)}%</div>
          </div>
          <div class="track-road">
            <div class="lane-glow" style="width:calc(${left}% + 16px)"></div>
            <img src="${player.carImage}" alt="${escapeHtml(player.carName)}" style="left:calc(${left}% + 14px)">
          </div>
        </article>`;
    }).join('');
  }
